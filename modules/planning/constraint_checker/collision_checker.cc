        ...
        const DiscretizedTrajectory& discretized_trajectory)
{
    ...
    for (size_t i = 0; i < discretized_trajectory.NumOfPoints(); ++i)
    {
        ...
        Box2d ego_box({trajectory_point.path_point().x(),
...
int generate_polygon_path(double safe_buffer,
                          ...
                          int path_check_size)
{
    ...
    for (int i = 0; i <= path_check_size; i++)
    {
        const PathPoint& point = path_points[i];
...
bool check_path_collision_with_static_obstacle(
        int* collision_index, const std::vector<Polygon2D>& polygon_path,
        const Obstacle& obstacle)
{
    *collision_index = -1;
    ...
    {
        const Polygon2D& point_polygon = polygon_path[i];
...
#if 0
    AINFO << "obs id: " << obstacle.Id() << " is collision: " << is_collision
          << " collision path point id: " << *collision_index;
...
bool check_path_collision_with_dynamic_obstacle_point(
        int* start_collision_index, int* end_collision_index,
        ...
        const Polygon2D& obs_polygon)
{
    ...
    *end_collision_index = -1;
    ...
    return is_first_collision;
}

bool is_path_collision_with_static_obstacle(
        const std::vector<Polygon2D>& polygon_path,
        const Polygon2D& obs_polygon)
{
