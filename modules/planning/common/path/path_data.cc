...
bool PathData::SetDiscretizedPath(DiscretizedPath path)
{
    ...
    if (discretized_path_.size() != frenet_path_.size())
    {
        return false;
    }
                ...
                path_point_decision_guide)
{
    if (reference_line_ == nullptr)
    {
        AERROR << "Should NOT set path_point_decision_guide when reference "
                  "line is nullptr. ";
        return false;
    }
    ...
    {
        AINFO << "Should NOT set path_point_decision_guide when frenet_path "
    ...
    for (const common::FrenetFramePoint &frenet_point : frenet_path)
    {
        ...
        if (!path_points.empty())
        {
            common::math::Vec2d last =
                    PointFactory::ToVec2d(path_points.back());
        ...
        path_points.push_back(PointFactory::ToPathPoint(
                cartesian_point.x(), cartesian_point.y(), 0.0, s, theta, kappa,
                dkappa));
...
bool PathData::LeftTrimWithRefS(const common::FrenetFramePoint &frenet_point)
{
    ...
    for (const common::FrenetFramePoint point : frenet_path_)
    {
        if (std::fabs(point.s() - frenet_point.s()) < 1e-6)
        {
            continue;
        }

        if (point.s() > frenet_point.s())
        {
            frenet_frame_points.push_back(point);
        }
    }
