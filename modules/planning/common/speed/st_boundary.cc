...
STBoundary STBoundary::ExpandLowerBoundByS(const double s) const
{
    if (lower_points_.empty())
    {
        return STBoundary();
...
STBoundary STBoundary::ExpandByT(const double t) const
{
    ...
    const double lower_left_delta_s =
            lower_points_[1].s() - lower_points_[0].s();
    const double upper_left_delta_s =
            upper_points_[1].s() - upper_points_[0].s();

    point_pairs.emplace_back(
            STPoint(lower_points_[0].s() -
                            t * lower_left_delta_s / left_delta_t,
                    lower_points_[0].t() - t),
            STPoint(upper_points_[0].s() -
                            t * upper_left_delta_s / left_delta_t,
        ...
        const std::vector<std::pair<STPoint, STPoint>>& point_pairs) const
{
    ...
    {
        AERROR << "point_pairs.size() must >= 2, but current "
                  "point_pairs.size() = "
               << point_pairs.size();
        return false;
    }
    ...
    for (size_t i = 0; i < point_pairs.size(); ++i)
    {
        ...
        {
            AERROR << "Points in every ST-point pair should be at the same "
                      "time.";
            return false;
        }
